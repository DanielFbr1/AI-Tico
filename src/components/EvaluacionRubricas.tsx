...
export function EvaluacionRubricas({ rubrica, grupos = [], proyectoId }: EvaluacionRubricasProps) {
    ...
    try {
      ...
      const excelData = allEvaluations.map((ev: any) => {
        ...
        return {
          ...
          'Nota Final': Number(notaFinalCalculada.toFixed(2)),
          ...criteriosObj
        };
      });
      ...
      utils.book_append_sheet(wb, wsGrades, "Calificaciones");
      utils.book_append_sheet(wb, wsRubric, "Rúbrica");
      writeFile(wb, "Calificaciones_y_Rubrica_TicoAI.xlsx");
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="space-y-2">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden mt-12">
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-[10px] text-slate-400 uppercase bg-slate-50/50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-4 font-black tracking-widest text-center bg-indigo-50/50 text-indigo-700">Nota Final</th>
              ...
              <tbody className="divide-y divide-slate-100 font-bold">
                {allEvaluations.sort((a, b) => (grupoMap[a.grupo_id] || '').localeCompare(grupoMap[b.grupo_id] || '') || a.alumno_nombre.localeCompare(b.alumno_nombre)).map((ev, idx) => {
                  ...
                  return (
                    <tr key={ev.id || `row-${idx}`} className="hover:bg-indigo-50/30 transition-colors group/row text-slate-600">
                      ...
                      {criteriosActivos.map((c, i) => {
                        ...
                        const getColor = (p: number) => {
                          if (p >= 9) return 'text-green-600';
                          if (p >= 7) return 'text-blue-600';
                          if (p >= 5) return 'text-yellow-600';
                          return 'text-red-600';
                        };
                        return (
                          <td key={`${ev.id || idx}-${c}-${i}`} className={`px-6 py-4 text-center font-black ${getColor(puntos)}`}>
